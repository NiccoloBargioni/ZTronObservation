...
fileprivate let logger = Logger(subsystem: "ZombieTronObservation", category: "LinkedList")
    ...
    public struct LinkedListIndex: Comparable {
        ...
        internal func removeFromOwnerList() {
            ...
            guard let owner = self.owner else {
                #if DEBUG
                logger.debug("In function \(#function): Attempted to remove an element from its nil owner list. Aborted.")
                #endif
                fatalError()
            }
            ...
            guard let node = self.node else {
                #if DEBUG
                logger.debug("In function \(#function): Attempted to remove nil node from its parent list. Aborted.")
                #endif
